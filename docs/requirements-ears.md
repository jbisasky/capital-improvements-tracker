...
| SEC-08 | Ubiquitous | The BYOK API key shall be restricted (in Google Cloud console) to the Gemini API only (service: `generativelanguage.googleapis.com`). Where the key type supports HTTP referrer restrictions (legacy standard keys), those shall also be applied; auth keys bound to a service account (2026 default from AI Studio) cannot use referrer restrictions and shall rely on Gemini-only API restriction, Google leaked-key enforcement, project quota caps, and app-level guards (LLD §13). This shall be documented in `docs/google-cloud-setup.md`. |
